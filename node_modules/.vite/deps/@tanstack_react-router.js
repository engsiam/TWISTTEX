...
function parseSegments(defaultCaseSensitive, data, route, start, node, depth, dynamicListsToSort, onRoute) {
  ...
  let cursor = start;
  {
    ...
    const caseSensitive = (options == null ? void 0 : options.caseSensitive) ?? defaultCaseSensitive;
    const parseParams = ((_a = options == null ? void 0 : options.params) == null ? void 0 : _a.parse) ?? (options == null ? void 0 : options.parseParams);
    while (cursor < length) {
      ...
      switch (kind) {
        case 0: {
          ...
          let name = value;
          let staticChildren;
          if (caseSensitive) staticChildren = node.static ?? (node.static = /* @__PURE__ */ new Map());
          else {
            name = value.toLowerCase();
            staticChildren = node.staticInsensitive ?? (node.staticInsensitive = /* @__PURE__ */ new Map());
          }
        ...
        case 2: {
          ...
          const existingNode = kind !== 2 && !parseParams && (siblings == null ? void 0 : siblings.find((s) => !s.parse && s.caseSensitive === actuallyCaseSensitive && s.prefix === prefix && s.suffix === suffix));
          if (existingNode) nextNode = existingNode;
          else {
            ...
            next.depth = depth;
            let nodes;
            if (kind === 1) nodes = node.dynamic ?? (node.dynamic = []);
            else if (kind === 3) nodes = node.optional ?? (node.optional = []);
            else nodes = node.wildcard ?? (node.wildcard = []);
            nodes.push(next);
            if (nodes.length === 2) dynamicListsToSort == null ? void 0 : dynamicListsToSort.push(nodes);
          }
          break;
        }
      }
    ...
    node.priority = ((_b = options == null ? void 0 : options.params) == null ? void 0 : _b.priority) ?? 0;
...
function createBrowserHistory(opts) {
  ...
  const queueHistoryAction = (isPush, destHref, state) => {
    ...
    next = [
      href,
      state,
      (next == null ? void 0 : next[2]) || isPush
    ];
...
function routeNeedsLoad(route) {
  var _a, _b;
  return route.options.loader || route.options.beforeLoad || route.lazyFn || ((_a = route.options.component) == null ? void 0 : _a.preload) || ((_b = route.options.pendingComponent) == null ? void 0 : _b.preload);
}
...
function getLocationChangeInfo(location, resolvedLocation) {
  return {
    ...
    toLocation: location,
    pathChanged: (resolvedLocation == null ? void 0 : resolvedLocation.pathname) !== location.pathname,
    hrefChanged: (resolvedLocation == null ? void 0 : resolvedLocation.href) !== location.href,
    hashChanged: (resolvedLocation == null ? void 0 : resolvedLocation.hash) !== location.hash
  };
}
...
function runRouteLifecycle(router, previous, matches, owner) {
  ...
  for (const match of previous) {
    ...
    if (!matches.some((candidate) => candidate.routeId === match.routeId)) (_b = (_a = router.routesById[match.routeId].options).onLeave) == null ? void 0 : _b.call(_a, match);
  }
  for (const match of matches) {
    if (owner && router._tx !== owner) return;
    (_d = (_c = router.routesById[match.routeId].options)[previous.some((candidate) => candidate.routeId === match.routeId) ? "onStay" : "onEnter"]) == null ? void 0 : _d.call(_c, match);
  }
}
var RouterCore = class {
  ...
  */
  constructor(options, getStoreConfig) {
    ...
    this.getMatchedRoutes = (pathname) => {
      ...
      return [
        (match == null ? void 0 : match.branch) || [this.routesById["__root__"]],
        rawParams,
        match == null ? void 0 : match.route
      ];
    };
    this.buildLocation = (opts) => {
      const build = (dest = {}) => {
        ...
        if (destRoute && !opts.leaveParams) try {
          ...
          if ((foundRoute == null ? void 0 : foundRoute.id) !== destRoute.id) console.warn(`Generated path "${nextPathname}" for route "${destRoute.id}" matched route "${foundRoute == null ? void 0 : foundRoute.id}" instead. This can happen when multiple route templates resolve to the same URL. Use the route template that matches the intended route, or adjust params.stringify if it changed the target path.`);
        } catch {
        }
    ...
    this.load = async (opts) => {
      ...
      this.updateLatestLocation();
      if (opts == null ? void 0 : opts.action) this._scroll.hash = opts.action.type === "PUSH" || opts.action.type === "REPLACE";
    ...
    this.startViewTransition = (fn) => {
      ...
      if (shouldViewTransition && !(isServer ?? typeof document === "undefined") && typeof document.startViewTransition === "function") {
        ...
        if (typeof shouldViewTransition === "object" && ((_b = (_a = window.CSS) == null ? void 0 : _a.supports) == null ? void 0 : _b.call(_a, "selector(:active-view-transition-type(a))"))) {
    ...
    this.invalidate = (opts) => {
      ...
      const invalidIds = new Set([
        ...committedMatches,
        ...this._cache.values(),
        ...[...(preloads == null ? void 0 : preloads.values()) ?? []].flat(),
        ...((_a = this._tx) == null ? void 0 : _a[3]) ?? []
      ...
      const invalidate = (d) => {
        if (invalidIds.has(d.id)) {
          ...
          const next = {
            ...d,
            invalid: true,
            ...((opts == null ? void 0 : opts.forcePending) || d.status === "error" || d.status === "notFound") && routeNeedsLoad(route) ? {
              status: "pending",
              error: void 0
            } : void 0
          };
      ...
      for (const [id, match] of this._cache) if (invalidIds.has(id)) {
        match.invalid = true;
        if (opts == null ? void 0 : opts.forcePending) match.status = "pending";
      }
      for (const id of invalidIds) (_b = this._flights) == null ? void 0 : _b.delete(id);
    ...
    this.clearCache = (opts) => {
      ...
      for (const match of discarded) {
        ...
        if (flight && !--flight[2]) {
          if (((_a = this._flights) == null ? void 0 : _a.get(match.id)) === flight) this._flights.delete(match.id);
          abort.push(flight[1]);
        }
      }
    ...
    this.matchRoute = (location, opts) => {
      ...
      if ((opts == null ? void 0 : opts.pending) && !isPending) return false;
      const baseLocation = (opts == null ? void 0 : opts.pending) ?? !isPending ? this.latestLocation : this.stores.resolvedLocation.get() || this.stores.location.get();
  ...
  matchRoutesInternal(next, opts) {
    ...
    const previousAt = (route, index) => {
      ...
      return (match == null ? void 0 : match.routeId) === route.id ? match : route === this.options.notFoundRoute ? committed.find((candidate) => candidate.routeId === route.id) : void 0;
    };
    let strictParams;
    for (let index = 0; index < matchedRoutes.length; index++) {
      ...
      let loaderDepsHash = "";
      try {
        loaderDeps = ((_b = (_a = route.options).loaderDeps) == null ? void 0 : _b.call(_a, { search: preMatchSearch })) ?? "";
        loaderDepsHash = loaderDeps ? JSON.stringify(loaderDeps) || "" : "";
      } catch (cause2) {
        if (opts == null ? void 0 : opts.throwOnError) throw cause2;
        searchError ?? (searchError = cause2);
      }
      ...
      const existingMatch = (opts == null ? void 0 : opts._rematerialize) ? void 0 : this._cache.get(matchId) ?? ((previousMatch == null ? void 0 : previousMatch.id) === matchId ? previousMatch : void 0);
      strictParams = (existingMatch == null ? void 0 : existingMatch._strictParams) ?? Object.assign(usedParams, strictParams);
      let paramsError;
      ...
      if (existingMatch) match = {
        ...
        match = {
          ...
          index,
          routeId: route.id,
          params: (previousMatch == null ? void 0 : previousMatch.params) ?? strictParams,
          ...
          paramsError,
          context: {},
          abortController: (opts == null ? void 0 : opts._controller) ?? new AbortController(),
          cause,
    ...
    for (let index = 0; index < matches.length; index++) {
      ...
      if (opts == null ? void 0 : opts._controller) match.context = {};
    }
    return matches;
  }
  /**
  ...
  */
  matchRoutesLightweight(location) {
    ...
    const lastStateMatchId = lastStateMatch == null ? void 0 : lastStateMatch.id;
...
function findGlobalNotFoundRouteId(notFoundMode, routes) {
  if (notFoundMode !== "root") {
    ...
    for (let i = routes.length - 1; i >= 0; i--) {
      ...
      fallback || (fallback = route.children && route.id);
    }
...
function normalize2(value, rejected, routeId) {
  ...
  if (isNotFound(value)) {
    value.routeId || (value.routeId = routeId);
    return [NOT_FOUND, value];
  }
  if (!rejected) return [SUCCESS, value];
  if (typeof (value == null ? void 0 : value.then) === "function") value = new Error("A Promise was thrown", { cause: value });
  return [ERROR, value];
}
function normalizeError(route, cause) {
  ...
  try {
    (_b = (_a = route.options).onError) == null ? void 0 : _b.call(_a, outcome[1]);
  } catch (onErrorCause) {
...
async function contextualize(router, lane, options, end, planSuccessfulLane, retainedEnd) {
  ...
  for (let index = options[6] ?? 0; index < end; index++) {
    ...
    const parentContext = ((_a = matches[index - 1]) == null ? void 0 : _a.context) ?? router.options.context ?? {};
    const common = {
      params: match.params,
      location,
    ...
    try {
      const routeContext = match._ctx || (match._ctx = route.options.context ? route.options.context({
        ...common,
    ...
    if (index >= retainedEnd) {
      ...
      (_b = options[7]) == null ? void 0 : _b.call(options);
    }
    try {
...
function releaseOwnedFlight(router, match, flight) {
  ...
  if (((_a = router._flights) == null ? void 0 : _a.get(match.id)) === flight) {
...
function releaseFlight(router, match) {
  ...
  const flight = match._flight;
  match._flight = void 0;
  (_a = releaseOwnedFlight(router, match, flight)) == null ? void 0 : _a.abort();
}
function transferMatchResources(router, previous, next, deferSameIdFlight) {
  var _a;
  const abort = [];
  for (const match of previous) if (!(next == null ? void 0 : next.includes(match))) {
    const flight = match._flight;
    match._flight = void 0;
    if (deferSameIdFlight && (flight == null ? void 0 : flight[2]) === 1 && ((_a = router._flights) == null ? void 0 : _a.get(match.id)) === flight && (next == null ? void 0 : next.some((candidate) => candidate.id === match.id))) flight[2] = 0;
    else {
...
function setFetching(router, match, value, owner) {
  ...
  if (owner && ((_a = router._tx) == null ? void 0 : _a[0]) !== owner) return;
  const store = router.stores.byRoute.get(match.routeId);
  const presented = store == null ? void 0 : store.get();
  if ((presented == null ? void 0 : presented.id) === match.id) store.set({
    ...presented,
    isFetching: value
  });
}
    ...
    if (!flight) {
      ...
      flight = [
        Promise.resolve().then(() => loader(getLoaderContext(router, lane, match, route, controller, parentMatchPromise, !!options[3]))).then((value) => normalize2(value, false, route.id), (cause) => normalize2(cause, true, route.id)).then((result) => {
          var _a;
          if (result[0] !== SUCCESS && ((_a = router._flights) == null ? void 0 : _a.get(match.id)) === flight) {
            router._flights.delete(match.id);
      ...
      (router._flights ?? (router._flights = /* @__PURE__ */ new Map())).set(match.id, flight);
    }
...
function createLoaderTask(router, lane, index, tasks, semanticParent, options, retainedEnd) {
  ...
  const loader = isLoaderFn ? routeLoader : routeLoader == null ? void 0 : routeLoader.handler;
  const preloadable = !preload || route.options.preload !== false;
  let donor = preloadable && routeLoader && !((_a = router._tx) == null ? void 0 : _a[6]) ? (_b = router._flights) == null ? void 0 : _b.get(match.id) : void 0;
  ...
  if (blocking) {
    ...
    match._flight = donor;
    (_c = releaseOwnedFlight(router, match, acceptedFlight)) == null ? void 0 : _c.abort();
...
async function getNotFoundBoundary(router, matches, indexed, signal, fallback = 0) {
  const cause = indexed == null ? void 0 : indexed[1][1];
  let index = (cause == null ? void 0 : cause.routeId) ? matches.findIndex((match) => match.routeId === cause.routeId) : (indexed == null ? void 0 : indexed[0]) ?? matches.length - 1;
  if (index < 0) index = 0;
  ...
  return (cause == null ? void 0 : cause.routeId) ? index : fallback;
}
...
async function reduceLane(router, lane, tasks, controller, settlement, onReady) {
  ...
  if (((failure == null ? void 0 : failure[1][0]) ?? 0) >= REDIRECTED) readinessEnd = 0;
  else if (failure) {
    readinessEnd = failure[2] ?? (failure[2] = await boundaryOf(failure));
  ...
  if (((failure == null ? void 0 : failure[1][0]) ?? 0) >= REDIRECTED) {
  ...
  if (boundary >= 0) {
    const outcome = failure == null ? void 0 : failure[1];
    const kind = outcome == null ? void 0 : outcome[0];
    const match = matches[boundary];
    const cause = outcome == null ? void 0 : outcome[1];
    const install = () => {
      if (outcome) {
    ...
    install();
    if (!outcome) onReady == null ? void 0 : onReady();
    const route = getRoute(router, match);
    try {
...
async function projectLane(router, lane, signal, start = 0, end = lane[1].length) {
  ...
  for (let index = start; index < end; index++) {
    ...
    if (routeOptions.head || routeOptions.scripts) try {
      ...
      const [head, scripts] = await waitFor(Promise.all([(_a = routeOptions.head) == null ? void 0 : _a.call(routeOptions, context), (_b = routeOptions.scripts) == null ? void 0 : _b.call(routeOptions, context)]), signal);
      match.meta = head == null ? void 0 : head.meta;
      match.links = head == null ? void 0 : head.links;
      match.headScripts = head == null ? void 0 : head.scripts;
      match.styles = head == null ? void 0 : head.styles;
...
async function executeClientLane(router, location, matches, options) {
  ...
  try {
    ...
    while (retainedEnd < end && retainedEnd !== plannedBoundary) {
      ...
      if ((committed == null ? void 0 : committed.id) !== match.id || committed.status !== "success" || committed._notFound || match.preload || (visible == null ? void 0 : visible.id) !== match.id || visible.status !== "success" || visible._notFound) break;
      retainedEnd++;
    }
    const tasks = [];
    ...
    if ((_a = matched[2]) == null ? void 0 : _a.length) matched[3] = settleTasks(matched[2], void 0, void 0, reduction.then((foreground) => isControl(foreground) ? 0 : _getRenderedMatches(matches).length, () => 0));
...
function offerPending(router, tx) {
  ...
  for (let index = 0; index < matches.length; index++) {
    ...
    const presentedPending = ((_a = presented[index]) == null ? void 0 : _a.id) === match.id && ((_b = presented[index]) == null ? void 0 : _b.status) === "pending";
    ...
    let tookOver = false;
    if ((session == null ? void 0 : session[1]) === match.id) {
      tookOver = session[0] !== tx;
      session[0] = tx;
    } else {
      clearTimeout(session == null ? void 0 : session[3]);
...
function finishPending(router, tx) {
  ...
  if (router._tx === tx || !((_a = router._tx) == null ? void 0 : _a[3].some((match) => match.id === (session == null ? void 0 : session[1])))) {
    clearTimeout(session == null ? void 0 : session[3]);
...
function commitMatches(router, tx, matches, resolvedPrefix) {
  ...
  if (true) {
    const handoff = (_a = tx[6]) == null ? void 0 : _a[0];
...
async function runBackground(router, tx, base, tasks, settlement) {
  ...
  for (const match of next) {
    ...
    if ((cached == null ? void 0 : cached._flight) && cached._flight === match._flight) {
...
async function runClientTransaction(router, tx, forceStaleReload, onReady, sync, resolvedPrefix) {
  ...
  await router.startViewTransition(async () => {
    ...
    if (background == null ? void 0 : background.length) runBackground(router, tx, matches, background, result[3]).catch(console.error);
    router.batch(() => {
    ...
    if (router._tx !== tx) return;
    (_a = router._commitPromise) == null ? void 0 : _a.resolve();
...
async function loadClientRoute(router, opts) {
  ...
  if (true) rematerialize = !!router._refreshNextLoad || !!((_a = router._tx) == null ? void 0 : _a[6]);
  ...
  const redirects = (pendingLocation == null ? void 0 : pendingLocation.href) === location.href ? pendingLocation._redirects ?? 0 : 0;
  const handoff = router._handoff;
  const hydrationController = rematerialize ? void 0 : handoff == null ? void 0 : handoff[0]();
  const preflight = new AbortController();
  const previousPreflight = router._preflight;
  router._preflight = preflight;
  if (!rematerialize && !hydrationController) handoff == null ? void 0 : handoff[1]();
  previousPreflight == null ? void 0 : previousPreflight.abort();
  ...
  else hydrationController == null ? void 0 : hydrationController.abort();
  if (preflight.signal.aborted) {
    transferMatchResources(router, matches);
    await awaitCurrent(router, previousOwner);
    return;
  ...
  let settle;
  const run = () => runClientTransaction(router, tx, sameHref, () => offerPending(router, tx), opts == null ? void 0 : opts.sync, resolvedPrefix);
  const done = (opts == null ? void 0 : opts.sync) ? new Promise((resolve) => settle = resolve) : Promise.resolve().then(run).then();
  const tx = [
    controller,
    redirects,
    location,
  ...
  if (resolvedPrefix || !router._committed.length && ((_b = matches[0]) == null ? void 0 : _b.status) !== "success" && !matches.some((match) => match._notFound)) offerPending(router, tx);
  settle == null ? void 0 : settle(run());
  await done;
  await awaitCurrent(router, tx);
}
async function refreshClientRoute(router) {
  ...
  (_a = router._flights) == null ? void 0 : _a.clear();
  router.clearCache();
...
async function preloadClientRoute(router, opts) {
  var _a, _b;
  if (router._refreshNextLoad || ((_a = router._tx) == null ? void 0 : _a[6])) return;
  ...
  for (let redirects = 0; ; redirects++) {
    ...
    try {
      try {
        ...
        acquireMatchResources(matches);
        active = (router._preloads ?? (router._preloads = /* @__PURE__ */ new Map())).set(controller, matches);
      ...
      if (!active || result.length < 3 || (router._refreshNextLoad || ((_b = router._tx) == null ? void 0 : _b[6]))) return;
      location = result[2];
    } catch (cause) {
...
function waitForReason(value, signal, onLate) {
  ...
  return new Promise((resolve, reject) => {
    ...
    signal.addEventListener("abort", abort, { once: true });
    promise.then((result) => {
      if (signal.aborted) onLate == null ? void 0 : onLate(result);
      else resolve(result);
...
function normalize3(value, rejected) {
  ...
  if (rejected && typeof (value == null ? void 0 : value.then) === "function") value = new Error("A Promise was thrown", { cause: value });
...
async function resolveSsr(router, lane, index) {
  ...
  const parentSsr = (_a = lane.matches[index - 1]) == null ? void 0 : _a.ssr;
...
async function contextualize2(router, lane, signal) {
  ...
  for (let index = 0; index < end; index++) {
    ...
    signal == null ? void 0 : signal.throwIfAborted();
    if ((failure == null ? void 0 : failure[1][0]) === REDIRECTED2) break;
...
function getLoaderContext2(router, lane, match, route, index, tasks) {
  ...
  return {
    ...
    deps: match.loaderDeps,
    preload: false,
    parentMatchPromise: (_a = tasks[index - 1]) == null ? void 0 : _a.match,
    abortController: match.abortController,
    context: match.context,
    location: lane.location,
    navigate: navigateFrom(router, lane.location),
...
function createLoaderTask2(router, lane, index, tasks, signal) {
  ...
  if (match.ssr === false) outcome = Promise.resolve([SKIPPED]);
    ...
    if (!loader) outcome = Promise.resolve([SUCCESS2, void 0]);
      ...
      if ((signal == null ? void 0 : signal.aborted) || match.abortController.signal.reason === lane) return [SKIPPED];
    ...
    if (routeOptions.head || routeOptions.scripts || routeOptions.headers) {
      ...
      try {
        const [head, scripts, headers] = await Promise.all([
          (_a = routeOptions.head) == null ? void 0 : _a.call(routeOptions, context),
          (_b = routeOptions.scripts) == null ? void 0 : _b.call(routeOptions, context),
          (_c = routeOptions.headers) == null ? void 0 : _c.call(routeOptions, context)
        ]);
        signal == null ? void 0 : signal.throwIfAborted();
        match.meta = head == null ? void 0 : head.meta;
        match.links = head == null ? void 0 : head.links;
...
async function executeServerLane(router, location, matchedMatches, signal) {
  ...
  const abortLane = () => abortMatches(matched.matches, 0, signal == null ? void 0 : signal.reason);
  if (signal == null ? void 0 : signal.aborted) {
    abortLane();
    signal.throwIfAborted();
  }
  signal == null ? void 0 : signal.addEventListener("abort", abortLane, { once: true });
  try {
    ...
    if (((_a = lane.failure) == null ? void 0 : _a[1][0]) === REDIRECTED2) loaderEnd = 0;
    else if (((_b = lane.failure) == null ? void 0 : _b[1][0]) === NOT_FOUND2) {
    ...
    let loaderFailure;
    let control = ((_c = lane.failure) == null ? void 0 : _c[1][0]) === REDIRECTED2 ? lane.failure : void 0;
    try {
    ...
    signal == null ? void 0 : signal.throwIfAborted();
    if ((control == null ? void 0 : control[1][0]) === REDIRECTED2) {
    ...
    let readinessEnd;
    if (failure) {
      const outcomeEnd = failure[2] ?? (failure[2] = failure[1][0] === NOT_FOUND2 ? await getNotFoundBoundary2(router, lane.matches, failure, signal) : failure[0]);
      for (const task of tasks) {
    ...
    signal == null ? void 0 : signal.throwIfAborted();
    (_d = router.serverSsr) == null ? void 0 : _d.onCleanup(abortLane);
    return {
      type: "render",
    ...
    signal == null ? void 0 : signal.removeEventListener("abort", abortLane);
  }
}
async function loadServerRoute(router, opts) {
  ...
  try {
    ...
    (_a = opts == null ? void 0 : opts._signal) == null ? void 0 : _a.throwIfAborted();
    result = await waitFor2(executeServerLane(router, next, router.matchRoutes(next), opts == null ? void 0 : opts._signal), opts == null ? void 0 : opts._signal);
    (_b = opts == null ? void 0 : opts._signal) == null ? void 0 : _b.throwIfAborted();
  } catch (cause) {
    (_c = opts == null ? void 0 : opts._signal) == null ? void 0 : _c.throwIfAborted();
  ...
  (_d = router._commitPromise) == null ? void 0 : _d.resolve();
  router._commitPromise = void 0;
}
...
var CatchBoundary = class extends React$14.Component {
  ...
  componentDidCatch(error, errorInfo) {
    var _a, _b;
    (_b = (_a = this.props).onCatch) == null ? void 0 : _b.call(_a, error, errorInfo);
  }
  render() {
...
function resolveIsActive(location, next, activeOptions, basepath, isHydrated, isExternal) {
  ...
  if (activeOptions == null ? void 0 : activeOptions.includeHash) return isHydrated && location.hash === next.hash;
  return true;
}
function useLinkProps(options, forwardedRef) {
  ...
  const selectLinkState = React$110.useCallback((location) => {
    ...
    return [
      hrefOption == null ? void 0 : hrefOption.href,
      externalLink2,
...
function renderPending(router, route) {
  const PendingComponent = (route == null ? void 0 : route.options.pendingComponent) ?? router.options.defaultPendingComponent;
...
function MatchView({ router, match }) {
  ...
  const ResolvedSuspenseBoundary = canWrapInSuspense(router, route, match.ssr) && (route.options.wrapInSuspense ?? pendingElement ?? (((_b = route.options.errorComponent) == null ? void 0 : _b.preload) || resolvedNoSsr)) ? React$112.Suspense : SafeFragment;
  ...
  return (0, import_jsx_runtime12.jsxs)(route.isRoot ? route.options.shellComponent ?? SafeFragment : SafeFragment, { children: [(0, import_jsx_runtime12.jsx)(matchContext.Provider, {
    ...
    children: (0, import_jsx_runtime12.jsx)(ResolvedSuspenseBoundary, {
      ...
      children: (0, import_jsx_runtime12.jsx)(ResolvedCatchBoundary, {
        ...
        onCatch: (error, errorInfo) => {
          if (isNotFound(error)) {
            error.routeId ?? (error.routeId = match.routeId);
            throw error;
          }
  ...
  }), (isServer ?? router.isServer) && ((_c = route.parentRoute) == null ? void 0 : _c.id) === rootRouteId && router.options.scrollRestoration ? (0, import_jsx_runtime12.jsx)(ScrollRestoration, {}) : null] });
}
var MatchInner = React$112.memo(function MatchInnerImpl({ match }) {
  ...
  const key = React$112.useMemo(() => {
    var _a;
    const remountDeps = (_a = route.options.remountDeps ?? router.options.defaultRemountDeps) == null ? void 0 : _a({
      routeId,
...
var Outlet = React$112.memo(function OutletImpl() {
  ...
  if (isServer ?? router.isServer) {
    ...
    childRouteId = (_a = matches[parentIndex + 1]) == null ? void 0 : _a.routeId;
  } else {
...
function settleOwner(owner, rendered) {
  ...
  owner.length = 0;
  settle == null ? void 0 : settle(rendered);
}
function Transitioner({ t }) {
  const router = useRouter();
  const acknowledgement = router._rendered ?? (router._rendered = []);
  ...
  useLayoutEffect2(() => {
    ...
    if (mounted == null ? void 0 : mounted.current) return unsub;
    ...
    const resolvedLocation = router.stores.resolvedLocation.get();
    if ((resolvedLocation == null ? void 0 : resolvedLocation.href) === location.href && resolvedLocation.state.__TSR_key === location.state.__TSR_key) acknowledgement.push(router.stores.matches.get(), (rendered) => {
...
function useMatchRoute() {
  ...
  return React$114.useCallback((opts) => {
    ...
    useStore(router.stores.resolvedLocation, (location) => location == null ? void 0 : location.href),
...
function Script({ attrs, children, preventScriptHoist }) {
  ...
  if (!hydrated) {
    if (attrs == null ? void 0 : attrs.src) return (0, import_jsx_runtime15.jsx)("script", {
      ...attrs,
      suppressHydrationWarning: true
    });
    if (typeof children === "string") return (0, import_jsx_runtime15.jsx)("script", {
...
var Scripts = () => {
  ...
  const getScripts = (matches) => {
    ...
    for (const match of matches) {
      const manifestScripts = (_b = manifest.routes[match.routeId]) == null ? void 0 : _b.scripts;
...
function HeadContent2(props) {
  ...
  return (0, import_jsx_runtime18.jsx)(import_jsx_runtime18.Fragment, { children: (hydrated ? tags.filter((tag) => {
    var _a2;
